...
public final class WebTopologySnapshot {
	...
	private static JsonObject buildTopology(ServerLevel level, String dimension, Simulator simulator) {
		...
		final RouteBindingsSavedData routeBindings = RouteBindingsSavedData.get(level);
		for (final Map.Entry<BlockPos, BlockPos> binding : routeBindings.getIndicatorBindings().entrySet()) {
			final BlockPos repeaterPos = binding.getKey();
			final BlockPos signalPos = binding.getValue();
			// Prefer the persisted type marker; retain the loaded-entity fallback for saves
			// created before the marker was introduced.
			if (!routeBindings.getRepeatingIndicatorBindings().contains(repeaterPos)
					&& !(level.getBlockEntity(repeaterPos) instanceof RepeatingSignalBlockEntity)) continue;
